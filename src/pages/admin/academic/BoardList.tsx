...
import { Plus, Search, BookOpen, Filter, Download, Upload } from 'lucide-react';
...
const BoardList: React.FC = () => {
  ...
  const handleMoveUp = (board: AcademicBoard, index: number) => {
    if (index === 0) return;
    const newBoards = [...boards];
    const globalIndex = boards.findIndex((b) => b.id === board.id);
    [newBoards[globalIndex - 1], newBoards[globalIndex]] = [newBoards[globalIndex], newBoards[globalIndex - 1]];
    newBoards[globalIndex - 1].position = globalIndex - 1;
    newBoards[globalIndex].position = globalIndex;
    setBoards(newBoards);
    toast.success('Position updated');
  };

  const handleMoveDown = (board: AcademicBoard, index: number) => {
    if (index === boards.length - 1) return;
    const newBoards = [...boards];
    const globalIndex = boards.findIndex((b) => b.id === board.id);
    [newBoards[globalIndex], newBoards[globalIndex + 1]] = [newBoards[globalIndex + 1], newBoards[globalIndex]];
    newBoards[globalIndex].position = globalIndex;
    newBoards[globalIndex + 1].position = globalIndex + 1;
    setBoards(newBoards);
    toast.success('Position updated');
